...
\begin{lem}
	Let $\mathcal{N}:\R^m\times(\R^m)^N\to\R^m$ denote the usual normalization transformation, i.e.,
	\begin{align*}
		\hat{z}&= \mathcal{N}(z)\\
	...
	and
	$$\sigma^2=\E[(z-\mu)^2]=\frac{1}{N}\sum_{j=1}^N(z_j-\mu)^2.$$
	Let $BN:\R^m\times\R^m\times\R^m\to\R^m$ denote the batch normalization transformation $z\mapsto\tilde{z}$, i.e.,
	\begin{align*}
		\tilde{z}&:=BN(\gamma,\beta,z)\\
		&=\gamma\odot\hat{z}+\beta\\
		&=\gamma\odot\frac{z-\mu}{\sqrt{\sigma^2+\epsilon}}+\beta.
	\end{align*}
	Then the differentials:
	\begin{itemize}
		\item $d(\mathcal{N})_z:T_z\R^m\to T_{\hat{z}}\R^m$ is given by
