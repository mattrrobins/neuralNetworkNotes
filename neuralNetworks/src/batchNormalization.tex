...
\subsection{The Normalization Operator}
...
$$\eta:\R^{m\times n}\to\R^{m\times n},\qquad\eta(\xi)=(x-\mu\vec{1}^T)\odot(\xi),$$
$$\theta:\R^{m\times n}\to\R^{m\times n},\qquad \theta(\xi)=(\sigma^2-\epsilon\vec{1})^{\odot-\frac{1}{2}}\vec{1}^T\odot(\xi),$$
$$\Theta:\R^{m\times n}\to\R^{m\times n},\qquad \Theta(\xi)=(\sigma^2-\epsilon\vec{1})^{\odot-\frac{3}{2}}\odot(\xi)$$
...
\end{align*}
To implement, let $T:=rN_x$ denote the rank $(2,2)$-tensor, then for basis vectors $\epsilon_k^l$ and $E_\rho^\lambda$, we compute for that
$$T=T{^k}{_l}{^\rho}{_\lambda}\epsilon^l_k\otimes E_\rho^\lambda,$$
\begin{align*}
	T{^i}{_j}{^\alpha}{_\beta}&=\ip{T(E_i^j),E_\alpha^\beta}\\
	&=\ip{T(}
\end{align*}
...
\subsection{$\mathcal{B}(\R^{m\times n},\R^{m\times n})$}

Suppose $T:\R^{m\times n}\to\R^{m\times n}$ is linear.  Then for $A\in\R^{m\times n}$, we have that
$$B=T(A),\qquad B{^i}{_j}=T{^\alpha}{_\beta}{^i}{_j}A_{\alpha\beta},$$
and
$$T=T^{\alpha\beta}{_{ij}}\epsilon_{\alpha\beta}\otimes E^{ij}.$$
Then
\begin{align*}
	T(E^{\mu\nu})&=T^{\alpha\beta}{_{ij}}\delta_\alpha^\mu\delta_\beta^\nu E^{ij}\\
	&=T^{\mu\nu}{_{ij}}E^{ij},
\end{align*}
and
\begin{align*}
	\ip{T(E^{\mu\nu}),E^{kl})}_F&=T^{\mu\nu}{_{ij}}\ip{E^{ij},E^{kl}}_F\\
	&=T^{\mu\nu}{_{ij}}\delta_{ac}\delta_{bd}\delta
\end{align*}
...
$$N(x_1,...,x_n)=(y_1,...,y_n),$$
where
$$y_j=\frac{x^i_j-\E[\pi^i(x)]}{\sqrt{\V[\pi^i(x)]+\epsilon}}.$$
Then for $(x,\xi)\in T(\R^m)^n$, we have that
$$dN_x(\xi)=\bigoplus_{j=1}^nd_jN_x(\xi_j).$$

For what follows, we fix $x\in(\R^m)^n$, $\alpha\in\{1,...,n\}$, and let $\xi\in T_{x_\alpha}\R^m$ and consider
$$d_\alpha N_x(\xi).$$
Furthermore, we fix $\beta\in\{1,...,n\}$, let $y=y_\beta:(\R^m)^n\to\R^m$ and only consider
$$d_\alpha y_x(\xi).$$
