...
and let $w\in\R^{1\times m}$, $b\in\R$, and let
$$a=\sigma(wx+b).$$
...
we have the sigmoid function $\sigma:\R\to(0,1)$ with $\sigma(z)=a$ and $\sigma'(z)=a(1-a)$, and we have the collection of affine-functionals $\phi_x:\R^{1\times m}\times\R\to\R$ given by
$$\phi_x(w,b)=wx+b,$$
for which we fix an arbitrary $x\in\R^m$ and write $\phi=\phi_x$, and set $z=\phi(w,b)$.  Finally, we introduce the auxiliary function $\mathcal{L}:\R^{1\times m}\times\R\to\R$ given by
$$\mathcal{L}(w,b)=\L(\sigma(\phi(w,b)),y).$$
Then by the chain rule, we have that
\begin{align*}
	d\mathcal{L}_{(w,b)}&=d_a\L_{(a,y)}\circ d\sigma_z\circ d\phi_{(w,b)}\\
...
\lstinputlisting[lastline=214]{src/python/logisticRegression/npLogisticRegression.py}
