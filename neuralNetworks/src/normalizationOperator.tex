...
\begin{enumerate}
	...
	$$\phi(x)=x^{\odot2}=x\odot x.$$
	  ...
	  \end{align*}
	  Next, we notice that if we let $\rho:=\E[\xi]\in\R^n$, then
	  \begin{align*}
	  	\rho\vec{1}^T&=\begin{pmatrix}
	  		\rho^1\\
	  		\vdots\\
	  		\rho^m
	  	...
	  	\end{pmatrix}\\
	  	&=\begin{bmatrix}
	  		\rho^1&\rho^1&\cdots&\rho^1\\
	  		\rho^2&\rho^2&\cdots&\rho^2\\
	  		\vdots&\vdots&\ddots&\vdots\\
	  		\rho^m&\rho^m&\cdots&\rho^m
	  	\end{bmatrix}\in\R^{n\times N}
	  \end{align*}
	  and hence that
	  \begin{align*}
	  	\E[(x-\mu\vec{1}^T)\odot\rho\vec{1}^T]&=\sum_{i=1}^n\left(\frac{1}{N}\sum_{j=1}^N(x^i_j-\mu^i)\rho^i\right)e_i\\
	  	&=\sum_{i=1}^n\left(\rho^i(\E[x^i]-\mu^i)\right)e_i\\
	  	&=0.
	  \end{align*}
	  ...
	  \end{align*}
	  We remark that for a fixed $x\in\R^{n\times N}$, we let $\sigma^2:=\V[x]$ denote the output.
\end{enumerate}
...
$$f:\R^{n\times N}\to\R^{n\times N},\qquad f(x)=\left(\V[x]+\vec{\epsilon}\right)^{\odot-\frac{1}{2}}\vec{1}^T,$$
and
$$g:\R^{n\times N}\to\R^{n\times N},\qquad g(x)=x-\E[x]\vec{1}^T.$$
However, we see here that $g\equiv\psi$ as defined, and so for any $(x,\xi)\in T\R^{n\times N}$, we have that
...
For what follows, we fix $x\in(\R^m)^n$, $j,\beta\in\{1,...,n\}$, and let $\xi\in T_{x_j}\R^m$ and let 
...
We note here that $rN_x$ is a rank $(2,2)$-tensor, and as such we need to compute its components if we wish to implement this in python.  To this end, let $\{E_\alpha^\beta\}$ denote the basis for $\R^{m\times n}$, where
